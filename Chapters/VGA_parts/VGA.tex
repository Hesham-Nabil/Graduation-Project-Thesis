...
\subsubsection*{PVT Calibration Framework}
...
\begin{figure}[H]
    \centering
    \framebox{\includegraphics[width=0.95\textwidth]{img/Termination_VGA_img/testbenchvga.pdf}}
...
\begin{figure}[H]
    \centering
    \framebox{\includegraphics[width=0.9\textwidth]{img/Termination_VGA_img/gain_steps.pdf}} % Replace with gain_steps.pdf
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}cc@{}}
        \toprule
        \textbf{Degeneration Control Code ($b_3 \dots b_0$)} & \textbf{Simulated Gain (dB)} \\ 
        \midrule
        0000 & 1.05 \\ 
        0001 & 2.54 \\ 
        0011 & 4.05 \\ 
...
\subsubsection*{VGA Linearity}
To ensure the PAM4 signal integrity is not compromised by amplifier compression, the large-signal linearity of the VGA was evaluated at both the minimum (1~dB) and maximum (7~dB) gain steps. Figure~\ref{fig:vga_linearity} presents the voltage transfer characteristics (S-curves) and the corresponding Total Harmonic Distortion (THD) profiles, verifying adequate voltage headroom across both extreme operating configurations.

\begin{figure}[H]
    \centering

    % --- Top Row: 7 dB Gain Step Configuration ---
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/Termination_VGA_img/7db_scurve.pdf} % Replace with 7db_scurve.pdf
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/Termination_VGA_img/THD_7db.pdf} % Replace with 7db_thd.pdf
    ...
    % --- Bottom Row: 1 dB Gain Step Configuration ---
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/Termination_VGA_img/1db_scurve.pdf} % Replace with 1db_scurve.pdf
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/Termination_VGA_img/THD_1db.pdf} % Replace with 1db_thd.pdf
...
A rigorous corner analysis was performed for every discrete gain step (1 dB, 2.5 dB, 4 dB, 5.5 dB, and 7 dB) to validate the effectiveness of the calibration networks. A total of 16 extreme scenarios were simulated for each state, encompassing extreme process variations (Slow-Slow and Fast-Fast), supply voltage fluctuations ($\pm 5\%$ VDD), and extreme industrial temperature ranges ($-40^\circ\text{C}$ to $125^\circ\text{C}$). 

By engaging the 1-bit $R_1$ load calibration in the SS corners and trimming the tail current via the 3-bit $I_{DAC}$, the VGA successfully maintained its target gain profile, bandwidth, and common-mode voltage across all severe PVT shifts without early capacitive roll-off or excessive peaking.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}ll@{}}
        \toprule
        \textbf{Performance Metric} & \textbf{Observed Variation} \\
        \midrule
        Common-Mode Voltage ($V_{CM}$) & $\sim \pm 20\,\text{mV}$ \\
        DC Gain Delta & $< \pm 0.6\,\text{dB}$ \\
...
\begin{figure}[H]
    \centering
    \framebox{\includegraphics[width=0.9\textwidth]{img/Termination_VGA_img/7db_postLayout.pdf}}
...
The input-referred and output noise profiles were evaluated at the maximum VGA and CTLE configuration (7 dB, -15dB respectively). This represents the worst-case noise scenario, as all degeneration branches are open, maximizing the thermal noise contribution of the source resistors. The integrated noise up to the 16 GHz operating bandwidth is documented in Table \ref{tab:chain_noise}.

\begin{figure}[H]
    \centering
    \framebox{\includegraphics[width=0.9\textwidth]{img/Termination_VGA_img/chain_noise.JPG}} % Replace with Chain Noise Plot
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \framebox{\includegraphics[width=\textwidth]{img/Termination_VGA_img/chain_thd.pdf}} % Replace with Chain THD Plot
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \framebox{\includegraphics[width=\textwidth]{img/Termination_VGA_img/chain_p1db.pdf}} % Replace with Chain P1dB Plot
