...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{SDR_Driver.png}
    \caption{SDR measurement of the TX driver, comparing Cadence simulation to the linear MATLAB reference.}
    \label{fig:sdr_driver}
\end{figure}
...
To further validate this methodology, a simplified RC-equivalent circuit model of the driver was constructed as shown in Figure~\ref{fig:sbr_tb}. By comparing the Single-Bit Response (SBR) of the full transistor-level circuit with this RC-model, we verify the fidelity of the model used to extract the Single Pulse Response (SPR) for the linear reference.
...
The SDR measurement of the RC-model, presented in Figure~\ref{fig:sdr_ideal}, yields an SDR of $51.96~\text{dB}$, which serves as a baseline for the idealized linear behavior. This confirms that the residual error observed in the full driver-level simulation is indeed representative of the device non-linearities rather than inaccuracies in the behavioral model.
